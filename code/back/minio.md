# springboot 集成minio 及使用示例
::: warning 注意
本机电脑必须先安装minio
安装教程:https://blog.csdn.net/DongJiYong_20/article/details/143179633
:::
1. 添加pom依赖
```java
    <dependency>
...
```
2. 在yml配置文件中添加minio相关信息
```java
minio:
  endpoint: minioAPI地址     ##示例：http://127.0.0.1:9425
  accesskey: 填写accesskey
  secret-key: 填写secret-key
  bucket: 添加bucket名称
  imgSize: 1024                           # 图片大小限制，单位：m
  fileSize: 1024                          # 文件大小限制，单位：m
```
然后获取accesskey和secret-key参考文章：https://blog.csdn.net/qq_33906471/article/details/139061820
3. 添加映射配置文件
```java
package com.bookmngsys.constants;
...
import com.bookmngsys.constants.MinioProp;
...
public class MinioConfiguration {
  ...
  System.out.println("Minio配置: endpoint=" + minioProp.getEndpoint() +
                     ", bucket=" + minioProp.getBucket() +
                     ", accesskey=" + minioProp.getAccesskey());
...
```java
    ...
    @ResponseBody
    public Response uploadImage(@RequestParam(value = "img", required = true) MultipartFile image) {
        try {
            // 检查 bucket 是否存在
            boolean found = minioClient.bucketExists(BucketExistsArgs.builder().bucket(minioProp.getBucket()).build());
            if (!found) {
                return new Response().failure("存储桶 'ljjk' 不存在，请先创建");
            }
            
            String fileName = UUID.randomUUID() + "." + image.getOriginalFilename().substring(image.getOriginalFilename().lastIndexOf(".") + 1);
            ...
            System.out.println("上传成功: " + result.get("url"));
            return new Response().success(result);
        } catch (Exception e) {
            e.printStackTrace();
            return new Response().failure("图片上传失败: " + e.getMessage());
        }
    }
```
